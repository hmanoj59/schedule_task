module IceCubeMethods
  ...
  CloudOptions = ['other','aws-naga-personal']
  RegionOptions = ['Choose a Region','us-east-1', 'us-west-2', 'us-west-1', 'eu-west-1', 'eu-central-1', 'ap-southeast-1', 'ap-northeast-1', 'ap-southeast-2', 'ap-northeast-2', 'ap-south-1', 'sa-east-1']
  RepeatEndsOptions = ['never','on']
  ActionOptions = [ 'Select an instance','start_rds', 'stop_rds', 'start_ec2', 'stop_ec2', 'Backup Ec2']
  RepeatMonthlyOptions = ['each','on']
  DaysOfWeeks =  {:choose_day_of_week => 8,
                  :monday=>1,
                  :tuesday=>2,
                  :wednesday=>3,
                  :thursday=>4,
                  :friday=>5,
                  :saturday=>6,
                  :sunday=>0} #(Date.today.beginning_of_week..Date.today.end_of_week).map {|w| w.strftime("%A")}
